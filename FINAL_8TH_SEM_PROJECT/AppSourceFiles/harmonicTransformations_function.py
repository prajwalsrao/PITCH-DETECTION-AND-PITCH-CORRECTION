...
def analysis(inputFile='../../sounds/vignesh.wav', window='blackman', M=1201, N=2048, t=-90, 
	...
	outputFile = '../outputSounds/' + os.path.basename(inputFile)[:-4] + '_harmonicModel.wav'
...
def transformation_synthesis(inputFile, fs, hfreq, hmag, freqScaling = np.array([0, 2.0, 1, .3]), 
    ...
    # convert all the lists to np array
	#hfreq = np.array(hfreq)
	#hmag = np.array(hmag)
	...
	# write output sound 
	outputFile = '../outputSounds/' + os.path.basename(inputFile)[:-4] + '_harmonicModelTransformation.wav'
	UF.wavwrite(y, fs, outputFile)
